...
task(){
	...
	TIMEST=`date -d "$DATE $TIME" "+%Y-%m-%dT%H:%M"` || TIMEST=0
        run_with_lock gdal_translate $file -sds -f XYZ /vsistdout/ -a_srs '+proj=stere +lat_0=90 +lon_0=0 +lat_ts=60 +a=6378.14 +b=6356.75 +x_0=0 y_0=0' | \
             awk '($3>0 && $3<65535) {printf("%d,%d,%d,%s\n"), $1, $2, $3, "'$TIMEST'"}' | \
             clickhouse-client \
	      --input_format_allow_errors_num=1 \
	      --input_format_allow_errors_ratio=0.1 \
	      --date_time_input_format=best_effort \
	      -d postgisday -q "INSERT INTO MFBSNL25_05m FORMAT CSV" || echo Empty input
}
